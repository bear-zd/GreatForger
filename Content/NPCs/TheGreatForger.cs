...
namespace GreatForger.Content.NPCs
{
	// [AutoloadHead] and NPC.townNPC are extremely important and absolutely both necessary for any Town NPC to work at all.
	[AutoloadHead]
	public class TheGreatForger : ModNPC
	{
		public override void SetStaticDefaults()
		{
			...
			NPC.Happiness
				.SetBiomeAffection<MushroomBiome>(AffectionLevel.Like) // Example Person prefers the forest.
				.SetBiomeAffection<UndergroundBiome>(AffectionLevel.Dislike) // Example Person dislikes the snow.
				.SetNPCAffection(NPCID.GoblinTinkerer, AffectionLevel.Love) // Loves living near the dryad.
		...
		public override bool CheckConditions(int left, int right, int top, int bottom)
		{
			...
			for (int x = left; x <= right; x++)
			{
				for (int y = top; y <= bottom; y++)
				{
					...
					if (Main.tile[x, y].WallType == WallID.Wood)
					{
						score++;
					}
				}
		...
		public override void OnChatButtonClicked(bool firstButton, ref bool shop)
		{
			if (firstButton)
			{
				...
				if (Main.LocalPlayer.HasItem(ItemID.LifeCrystal))
				{
					...
					Main.LocalPlayer.inventory[LifeCrystalItemIndex].TurnToAir();
					Main.LocalPlayer.QuickSpawnItem(entitySource, ModContent.ItemType<Items.Placeables.Ruam>());
			...
			{ Main.playerInventory = true;
              ...
              Main.npcChatText = "I'm not sure what to say to that.";

			GetInstance<GreatForger>().GreatForgerUserInterface.SetState(new GreatForgerUI());
	...
	public class ExamplePersonProfile : ITownNPCProfile
	{
		...
		public Asset<Texture2D> GetTextureNPCShouldUse(NPC npc)
		{
			if (npc.IsABestiaryIconDummy && !npc.ForcePartyHatOn)
				return ModContent.Request<Texture2D>("GreatForger/Content/NPCs/TheGreatForger");

			//if (npc.altTexture == 1)
			//	return ModContent.Request<Texture2D>("GreatForger/Content/NPCs/ExamplePerson_Party");

			return ModContent.Request<Texture2D>("GreatForger/Content/NPCs/TheGreatForger");
		}

		public int GetHeadTextureIndex(NPC npc) => ModContent.GetModHeadSlot("GreatForger/Content/NPCs/TheGreatForger_Head");
	}
}